...
function bumpCost(
  paperId: string,
  stage: keyof CostReport["breakdown"]["by_stage"],
  rawAmount: number,
  tokenDelta: Partial<CostReport["breakdown"]["llm_tokens"]> = {}
) {
  const c = costs.get(paperId) ?? emptyCost();
  c.breakdown.by_stage[stage] = round(c.breakdown.by_stage[stage] + rawAmount);
  (Object.keys(tokenDelta) as Array<keyof typeof tokenDelta>).forEach((k) => {
    c.breakdown.llm_tokens[k] += tokenDelta[k] ?? 0;
  });
  c.running_raw_usd = round(
    Object.values(c.breakdown.by_stage).reduce((a, b) => a + b, 0)
  );
